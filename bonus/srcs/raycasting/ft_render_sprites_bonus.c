...
static void	ft_get_sprite_info(t_cub *cub, int i)
{
	...
	sprite_height = (int)HEIGHT / transform_z; //do we need the fabs function? //using 'transform_y' instead of the real distance prevents fisheye
	...
	if (screen_w >= 0 && screen_w <= WIDTH && transform_z > 0)
	{
		...
		printf("start_w = %d, end_w = %d, diff_w = %d\n", sprite_start_w, sprite_end_w, sprite_end_w - sprite_start_w); //debug
		printf("start_h = %d, end_h = %d, diff_h = %d\n\n", sprite_start_h, sprite_end_h, sprite_end_h - sprite_start_h); //debug
	}
